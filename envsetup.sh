...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/ryuos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/ryuos/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function _lunch_meat()
{
    ...
    if (echo -n $1 | grep -q -e "^ryu_") ; then
        RYUOS_BUILD=$(echo -n $1 | sed -e 's/^ryu_//g')
    else
        RYUOS_BUILD=
    fi
    export RYUOS_BUILD
    ...
    if [[ -z "${ANDROID_QUIET_BUILD}" && -z "${RYUOS_BUILD}" ]]; then
...
function vosupload() {
    ...
    destination="${sf_username}@frs.sourceforge.net:/home/frs/project/romi-build/ROM/$target_device/"
...
. $ANDROID_BUILD_TOP/vendor/ryuos/build/envsetup.sh
